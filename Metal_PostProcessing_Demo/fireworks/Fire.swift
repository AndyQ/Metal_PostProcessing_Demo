...
class Firework : Drawable {
    ...
    init(time: Int64) { //}, aspect_x: Float) {
        let pos_x = random_range(lower:-20.0, 20.0)
        let pos_y = random_range(lower:10.0, 25.0)
        let pos_z = random_range(lower:80.0, 130.0)
...
//        self.add_flares(aspect_x: aspect_x)
    ...
    private func add_flares() {//aspect_x: Float) {
        ...
        for _ in 0..<count {
            ...
            let size = random_range(lower:0.06, 0.1)
